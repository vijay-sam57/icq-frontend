...
const Admin = () => {
  ...
  return (
    <div className="Admin">
        <Navbar user="Admin"/>
        { userForm ? <div className='container-fluid my-4 w-75 border border-2 border-secondary rounded-3 d-flex justify-content-center'>
        <form onSubmit={handleSubmit} className='w-50'>
            <h4 className='my-4'>Add User</h4>
                ...
                <div className="form-floating my-4 w-75 mx-auto">
                    <input type="password" name="password" id="password" placeholder='Password' className='form-control' value={password} onChange={(e)=>setPassword(e.target.value)}/>
        ...
        <div className="d-flex flex-column border border-secondary border-2 rounded-3 w-75 mx-auto my-5">
          ...
          <div className="btn-group">
            <button onClick={()=>setUserForm(true)} className="btn btn-outline-success btn-sm mx-2 px-3 rounded">Add User</button>
          </div>
          </div>
          {users.map((user,index)=>{
            return <User key={index} name={user} index={index} updateUser={updateUser} deleteUser={deleteUser}></User>
          })}
        </div>
    </div>
  )
